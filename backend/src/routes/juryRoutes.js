...
router.post("/", authMiddleware, checkRole(['Super-admin', 'Admin']), uploadFields, processS3Uploads, validate(jurySchema), createJury);
router.get("/", getAllJury);
router.get("/:id", getJuryById);
router.put("/:id", authMiddleware, checkRole(['Super-admin', 'Admin']), uploadFields, processS3Uploads, validate(jurySchema), updateJury);
